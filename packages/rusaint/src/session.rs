...
mod native {
    ...
    pub struct USaintSession(pub(crate) CookieStoreRwLock);
...
mod wasm {
    ...
    /// u-saint 로그인이 필요한 애플리케이션 사용 시 애플리케이션에 제공하는 세션 (WASM)
    #[derive(Debug, Default)]
    ...
    impl USaintSession {
        ...
        /// Set-Cookie 헤더들을 파싱하여 쿠키를 저장합니다.
                ...
                if let Ok(cookie_str) = value.to_str() {
                    ...
                    if let Some(name_value) = cookie_str.split(';').next() {
                        if let Some((name, value)) = name_value.split_once('=') {
                            ...
                            // Check if domain is specified in cookie
        ...
        pub async fn with_token(id: &str, token: &str) -> Result<USaintSession, RusaintError> {
            ...
            // Build cookie header for SSO request
            ...
            let req = client
                .get(format!("{SSU_USAINT_SSO_URL}?sToken={token}&sIdno={id}"))
                .headers(default_header())
                .header(COOKIE, cookie_header.parse::<HeaderValue>().unwrap())
                .header(HOST, "saint.ssu.ac.kr".parse::<HeaderValue>().unwrap())
                .build()
                .map_err(|e| {
                    WebDynproError::from(ClientError::FailedRequest(format!(
    ...
    pub async fn obtain_ssu_sso_token(id: &str, password: &str) -> Result<String, SsuSsoError> {
        ...
        // First request to get login form - manually track cookies
        ...
        // Extract cookies from response for subsequent request
        ...
        // Look for sToken in Set-Cookie headers
        let cookie_token = res
            ...
            .find_map(|value| {
                if let Ok(cookie_str) = value.to_str() {
                    if let Some(name_value) = cookie_str.split(';').next() {
                        if let Some((name, value)) = name_value.split_once('=') {
                            if name.trim() == "sToken" && !value.trim().is_empty() {
